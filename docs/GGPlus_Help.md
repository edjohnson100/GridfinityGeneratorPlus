...
If you've used the original GridfinityGenerator, here's what's different:
...
Every part generated with this add-in remembers its own settings — the exact parameters you used are stored directly on the component as a Fusion document attribute, not in a separate file or database. That means:
