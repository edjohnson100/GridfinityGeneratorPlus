...
There is no build system, package manager, linter, or test suite in this repo — it only runs inside Fusion 360's embedded Python interpreter via the Fusion 360 add-in loader. There are no `requirements.txt`/`pyproject.toml`. This is not currently a git repository.
...
- **`commandGridfinityPalette/`** — the active, current UI. A single palette (`adsk.core.Palettes`), tab-switched between "Bin" and "Baseplate", with explicit "Update Preview"/"Generate" buttons (no always-on live preview). `commands/__init__.py` registers only this command now — the old two are no longer wired up to run, just not yet deleted from disk. Structure:
  - `entry.py` — palette lifecycle: `start()`/`stop()`, `open_palette()`, `HTMLEventHandler` (dispatches JS→Python actions: `get_defaults`, `validate`, `update_preview`, `generate`, `save_defaults`, `reset_defaults`, `factory_reset`), `PaletteCloseHandler`, `_ensure_safe_to_mutate()` modal-safety guard.
  - `generation.py` — `create_and_build(kind, form)` / `build_bin(...)` / `build_baseplate(...)`, ported near-verbatim from the old dialogs' `generateBin`/`generateBaseplate`, now driven by plain form dicts instead of `CommandInputs`.
  - `validation.py` — `validate_bin(form)` / `validate_baseplate(form)` → `{valid, fieldErrors, computed}`, ported from the old dialogs' range checks.
  - `previewState.py` — single-slot preview tracker (module-level `_preview_tab`/`_preview_token`, re-resolved via `Design.findEntityByToken`); `track_preview()`/`clear_preview()`/`has_preview()`. Only one preview component exists at a time across both tabs.
  ...
  - `commandConfig/config.ini`, `commandConfig/ui_defaults.json` (`{"bin": {...}, "baseplate": {...}}`), `commandConfig/presets/{bin,baseplate}/*.json` — migrated from the old per-command config files.
  - `resources/palette/` — `index.html` / `style.css` / `script.js`, the actual palette UI. Field inputs use `id="{tab}.{fieldName}"`. `script.js` owns mm↔cm display conversion (Python/Fusion internal unit is cm) via a `MM_FIELDS` allowlist, bridges to Python via `window.adsk.fusionSendData` / `window.fusionJavaScriptHandler.handle`, and never uses `localStorage` (all durable state is Python-owned JSON pushed via a `set_state` message).
